...
<script>
//1. 安装echarts  2. 导入echarts (Echarts使用的第一种方式)
...
export default {
  ...
  mounted() {
    ...
    api.getChinaData().then((res) => {
      ...
      console.log("城市的数据", arr);
       //vue里面 等待延迟加载 直接读取是读取不到DOM元素"main"的 因为是第三方库vant里面的元素 按需加载
       this.$nextTick(()=>{
            //显示累计地图
  ...
  methods: {
    ...
    //   console.log("-----", title);
      if (title == 1) {
        this.$nextTick(() => {
          this.$myChart.chinaMap("main", this.arr); //Tab栏切换是通过元素的显示和隐藏 切换过去后才能拿到DOM元素
        });
      }
    },
    // getBar() {
